...
int find_start_frame(bool[], int);          // Нахождение стартового кадра
...
int main()
{
	...
	while ((start_data_package_index = find_start_frame(bit_data_stream, start_iterator)) >= 0)
	{
		/* Вывод индекса стартового кадра */
		printf("The start frame index is %d\n", start_data_package_index);
		...
		/* Вывод всей информации из пакета данных */
		printf("package is: \n");
		for (int i = 0; i < 8; i++)
		{
			for (int j = 0; j < 8; j++)
		...
		/* Увеличение итератора начала пакета на 64 бита */
		start_iterator += PACKAGE_SIZE * 8;
	}
...
int find_start_frame(bool stream[], int start_stream_index)
{
	int j = 0;
	for (int i = start_stream_index/8; i < DATA_LENGTH/8; i++)
	{
		for (j = 0; j < 8; j++)
		{
